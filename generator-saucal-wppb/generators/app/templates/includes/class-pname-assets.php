 ...
 */
abstract class <%= pluginShortClass %>_Assets {
	...
	protected function localize_asset( $path ) {
		$assets_path = <%= pluginSingleton %>()->plugin_path() . '/assets/';
		$assets_path_url = str_replace( array( 'http:', 'https:' ), '', <%= pluginSingleton %>()->plugin_url() ) . '/assets/';
	 ...
	 */
	private function register_script( $handle, $path, $deps = array( 'jquery' ), $version = <%=pluginShortNameUpper %>_VERSION, $in_footer = true ) {
		$this->scripts[] = $handle;
	 ...
	 * @param  boolean  $in_footer
	 */
	private function enqueue_script( $handle, $path = '', $deps = array( 'jquery' ), $version = <%=pluginShortNameUpper %>_VERSION, $in_footer = true ) {
	 ...
	 * @param  string   $media
	 */
	private function register_style( $handle, $path, $deps = array(), $version = <%=pluginShortNameUpper %>_VERSION, $media = 'all' ) {
		$this->styles[] = $handle;
	 ...
	 * @param  string   $media
	 */
	private function enqueue_style( $handle, $path = '', $deps = array(), $version = <%=pluginShortNameUpper %>_VERSION, $media = 'all' ) {
	...
	public function load_scripts() {
		...
		if ( ! did_action( 'before_<%= plugin_name %>_init' ) ) {
			return;
		}

		$suffix               = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
		$assets_path          = str_replace( array( 'http:', 'https:' ), '', <%= pluginSingleton %>()->plugin_url() ) . '/assets/';
		...
		if ( $enqueue_scripts = $this->get_scripts() ) {
			foreach ( $enqueue_scripts as $handle => $args ) {
				$args = wp_parse_args( $args, array(
					'src'       => '',
					'deps'      => array( 'jquery' ),
					'version'   => <%=pluginShortNameUpper %>_VERSION,
					'in_footer' => true,
				) );
